...
describe('Public Cash Calculator', () => {
  ...
  it('serves indexable homepage and clean calculator URLs', () => {
    const app = readFileSync(resolve('src/App.tsx'), 'utf8');
    ...
    expect(mainHtml).toContain('<meta name="robots" content="index, follow, max-image-preview:large"');
    expect(mainHtml).toContain('<link rel="canonical" href="https://www.hubvault.in/"');
  ...
  it('serves a functional and indexable COD reconciliation calculator', () => {
    const page = readFileSync(resolve('src/pages/public/CodReconciliationCalculator.tsx'), 'utf8');
    const html = readFileSync(resolve('cod-reconciliation-calculator.html'), 'utf8');
    const sitemap = readFileSync(resolve('public/sitemap.xml'), 'utf8');
    const vercel = JSON.parse(readFileSync(resolve('vercel.json'), 'utf8')) as {rewrites:Array<{source:string;destination:string}>};
    expect(html).toContain('<meta name="robots" content="index, follow"');
    expect(html).toContain('<link rel="canonical" href="https://www.hubvault.in/tools/cod-reconciliation-calculator"');
