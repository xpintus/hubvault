...
import { CalendarDays,Check,Clipboard,FileDown,Minus,Printer,RotateCcw,Share2,Sparkles,Target,UserRound } from 'lucide-react';
    ...
    <section className="relative min-h-screen overflow-hidden bg-[#f5f7ff] pb-28 dark:bg-[#080b16] md:pb-12">
      ...
      <div className="relative mx-auto max-w-7xl px-3 py-5 sm:px-6 sm:py-8 lg:px-8 lg:py-12">
        ...
        <div className="mt-6 grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px]">
          <div className="rounded-[28px] border border-white bg-white/80 p-3 shadow-xl shadow-slate-200/60 backdrop-blur dark:border-white/10 dark:bg-slate-900/70 dark:shadow-none sm:p-5"><div className="flex items-center justify-between px-1 pb-4"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-brand-600">Denominations</p><h2 className="mt-1 text-xl font-black text-slate-900 dark:text-white">Tap, type & count</h2></div><button disabled={totalNotes===0&&!name&&!expected} onClick={reset} className="inline-flex h-10 items-center gap-2 rounded-xl px-3 text-xs font-bold text-slate-500 transition hover:bg-slate-100 disabled:opacity-30 dark:hover:bg-white/10"><RotateCcw className="h-4 w-4"/>Reset</button></div>
