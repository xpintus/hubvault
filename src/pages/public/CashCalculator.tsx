...
export default function CashCalculator() {
  ...
  return <><SEO title="Free Cash Calculator - HubVault" description="Cash denomination calculator with optional reconciliation, WhatsApp summary, thermal printing and PDF download." path="/tools/cash-calculator" />
    <section className="min-h-screen bg-[#F8FAFC] py-6 pb-28 dark:bg-[#0F172A] md:min-h-[calc(100vh-4rem)] md:py-10 md:pb-10 lg:py-16"><div className="mx-auto max-w-6xl px-3 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-3xl text-center"><span className="inline-flex items-center gap-2 rounded-full border border-brand-600/20 bg-brand-600/10 px-4 py-1.5 text-xs font-semibold text-brand-600 dark:text-brand-400"><Banknote className="h-4 w-4"/>Free Public Tool</span><h1 className="mt-4 text-3xl font-bold text-neutral-900 dark:text-neutral-100 sm:text-4xl">Cash Reconciliation Calculator</h1><p className="mt-3 text-neutral-500 dark:text-neutral-400">Fast denomination counting, optional variance matching and shareable branded summaries.</p></div>

      <div className="mt-8 rounded-2xl border border-neutral-200 bg-white p-4 shadow-soft dark:border-neutral-800 dark:bg-neutral-900 sm:p-5"><div className="grid gap-4 md:grid-cols-3"><label className="text-sm font-semibold text-neutral-700 dark:text-neutral-300">Name<input value={name} onChange={e=>setName(e.target.value)} placeholder="Name" className="mt-1.5 h-11 w-full rounded-xl border border-neutral-200 bg-white px-3 font-normal outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-500/20 dark:border-neutral-700 dark:bg-neutral-950"/></label><label className="text-sm font-semibold text-neutral-700 dark:text-neutral-300">Date<input type="date" value={date} onChange={e=>setDate(e.target.value)} className="mt-1.5 h-11 w-full rounded-xl border border-neutral-200 bg-white px-3 font-normal outline-none focus:border-brand-500 dark:border-neutral-700 dark:bg-neutral-950"/></label><label className="text-sm font-semibold text-neutral-700 dark:text-neutral-300">Expected Amount<input type="number" min={0} value={expected} onChange={e=>setExpected(e.target.value)} placeholder="₹0" className="mt-1.5 h-11 w-full rounded-xl border border-neutral-200 bg-white px-3 font-normal outline-none focus:border-brand-500 dark:border-neutral-700 dark:bg-neutral-950"/></label></div></div>

      <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_360px]"><div className="overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-soft dark:border-neutral-800 dark:bg-neutral-900"><div className="hidden grid-cols-[110px_1fr_150px] gap-2 border-b border-neutral-200 bg-neutral-50 px-3 py-3 text-xs font-bold uppercase text-neutral-500 dark:border-neutral-800 dark:bg-neutral-950 sm:grid"><span>Note</span><span>Fast Quantity Input</span><span className="text-right">Amount</span></div>{NOTES.map((note,index)=><div key={note} className="grid grid-cols-[1fr_auto] items-center gap-x-3 gap-y-2 border-b border-neutral-100 px-3 py-3 last:border-0 dark:border-neutral-800 sm:grid-cols-[110px_1fr_150px] sm:gap-2 sm:py-2.5"><span className="font-bold text-brand-600 dark:text-brand-400">₹{note}</span><div className="col-span-2 row-start-2 flex min-w-0 items-center gap-2 sm:col-span-1 sm:col-start-2 sm:row-start-1 sm:gap-1"><button aria-label={`Decrease ₹${note}`} onClick={()=>adjust(note,-1)} className="flex h-10 w-11 shrink-0 items-center justify-center rounded-lg border border-neutral-200 dark:border-neutral-700 sm:w-10"><ChevronDown className="h-4 w-4"/></button><input data-note-index={index} aria-label={`₹${note} note quantity`} type="number" inputMode="numeric" min={0} value={counts[note]||''} placeholder="0" onChange={e=>setCount(note,e.target.value)} onKeyDown={e=>focusNext(e,index)} className="h-10 min-w-0 flex-1 rounded-lg border border-neutral-200 bg-white px-2 text-center font-bold outline-none focus:border-brand-500 dark:border-neutral-700 dark:bg-neutral-950"/><button aria-label={`Increase ₹${note}`} onClick={()=>adjust(note,1)} className="flex h-10 w-11 shrink-0 items-center justify-center rounded-lg border border-neutral-200 dark:border-neutral-700 sm:w-10"><ChevronUp className="h-4 w-4"/></button></div><span className="col-start-2 row-start-1 text-right font-semibold tabular-nums text-neutral-800 dark:text-neutral-200 sm:col-start-3">{money(note*counts[note])}</span></div>)}</div>

        <aside className="h-fit rounded-2xl bg-gradient-to-br from-brand-600 to-brand-900 p-5 text-white shadow-card-hover sm:p-6 lg:sticky lg:top-24"><div className="flex items-center justify-between gap-3"><p className="font-semibold">Cash Summary</p>{hasExpected&&<span className={`rounded-full border px-3 py-1 text-xs font-bold ${statusStyles[reconciliation.status]}`}>{reconciliation.status.toUpperCase()}</span>}</div><div className="mt-5 grid grid-cols-2 gap-3">{hasExpected&&<Metric label="Expected" value={money(expectedAmount)}/>}<Metric label="Collected" value={money(collected)}/>{hasExpected&&<Metric label="Variance" value={`${reconciliation.variance<0?'-':reconciliation.variance>0?'+':''}${money(reconciliation.variance)}`}/>}<Metric label="Total Notes" value={String(totalNotes)}/></div><div className="mt-5 grid gap-2"><button disabled={totalNotes===0} onClick={copyWhatsApp} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-white px-4 text-sm font-bold text-brand-700 disabled:opacity-40">{copied?<Check className="h-4 w-4"/>:<Clipboard className="h-4 w-4"/>}{copied?'WhatsApp Summary Copied':'Copy WhatsApp Summary'}</button><button disabled={totalNotes===0} onClick={shareReceipt} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-white/25 bg-white/10 px-4 text-sm font-semibold disabled:opacity-40"><Share2 className="h-4 w-4"/>Share Summary</button><button disabled={totalNotes===0} onClick={downloadPdf} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-white/25 bg-white/10 px-4 text-sm font-semibold disabled:opacity-40"><FileDown className="h-4 w-4"/>Download PDF</button><div className="grid grid-cols-2 gap-2"><button disabled={totalNotes===0} onClick={()=>printThermal(58)} className="inline-flex min-h-11 items-center justify-center gap-1 rounded-xl border border-white/25 bg-white/10 px-2 text-xs font-semibold disabled:opacity-40"><Printer className="h-4 w-4"/>2-inch Slip</button><button disabled={totalNotes===0} onClick={()=>printThermal(80)} className="inline-flex min-h-11 items-center justify-center gap-1 rounded-xl border border-white/25 bg-white/10 px-2 text-xs font-semibold disabled:opacity-40"><Printer className="h-4 w-4"/>3-inch Slip</button></div><button disabled={totalNotes===0&&!name&&!expected} onClick={reset} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl px-4 text-sm font-semibold text-white/75 disabled:opacity-40"><RotateCcw className="h-4 w-4"/>Reset All</button></div></aside>
      </div>
      <div className="fixed inset-x-3 bottom-3 z-40 flex items-center justify-between gap-3 rounded-2xl bg-neutral-950 px-4 py-3 text-white shadow-2xl md:hidden"><div className="min-w-0"><p className="text-[10px] uppercase text-white/50">Collected</p><p className="truncate text-lg font-bold tabular-nums">{money(collected)}</p></div>{hasExpected?<div className="shrink-0 text-right"><span className={`inline-flex rounded-full border px-2.5 py-1 text-[10px] font-bold ${statusStyles[reconciliation.status]}`}>{reconciliation.status.toUpperCase()}</span><p className="mt-1 text-xs text-white/70">Var: {reconciliation.variance<0?'-':reconciliation.variance>0?'+':''}{money(reconciliation.variance)}</p></div>:<div className="shrink-0 text-right"><p className="text-[10px] uppercase text-white/50">Total Notes</p><p className="font-bold tabular-nums">{totalNotes}</p></div>}</div>
    </div></section></>;
}
