...
const QUANTITY_WORDS:Record<string,number>=Object.fromEntries([...SMALL_WORDS.map((word,index)=>[word,index]),...TENS_WORDS.map((word,index)=>[word,index*10]).filter(([word])=>word)]);
export const parseVoiceCashCommand=(transcript:string):{note:NoteValue;quantity:number}|null=>{const text=transcript.toLowerCase().replace(/-/g,' ').replace(/\s+/g,' ').trim();const aliases:[NoteValue,RegExp][]=[[500,/\b(?:five hundred|500)\b/],[200,/\b(?:two hundred|200)\b/],[100,/\b(?:one hundred|hundred|100)\b/],[50,/\b(?:fifty|50)\b/],[20,/\b(?:twenty|20)\b/],[10,/\b(?:ten|10)\b/],[5,/\b(?:five|5)\b/],[2,/\b(?:two|2)\b/],[1,/\b(?:one|1)\b/]];for(const [note,pattern] of aliases){const match=text.match(pattern);if(!match)continue;const remainder=text.replace(pattern,' ').replace(/\b(?:rupees?|notes?|quantity|set|add|for|of)\b/g,' ').trim();const numeric=remainder.match(/\d+/);if(numeric)return {note,quantity:Math.max(0,Number(numeric[0]))};const parts=remainder.split(' ').filter(Boolean);let quantity=0;for(const part of parts){if(part==='hundred')quantity*=100;else quantity+=QUANTITY_WORDS[part]??0;}if(quantity>0)return {note,quantity};}return null;};
...
export default function CashCalculator() {
  ...
  const announce=(amount:number)=>{if(amount<=0||!('speechSynthesis' in window))return;window.speechSynthesis.cancel();const utterance=new SpeechSynthesisUtterance(`${numberWords(amount)} rupees added`);utterance.lang='en-IN';utterance.rate=.92;utterance.volume=.7;window.speechSynthesis.speak(utterance);};
  const focusNext=(event:KeyboardEvent<HTMLInputElement>,index:number,note:NoteValue)=>{
    if(event.key==='Enter'){announce(note*counts[note]);(document.querySelector(`[data-note-index="${index+1}"]`) as HTMLInputElement|null)?.focus();}
  };

  const toggleVoice=()=>{if(listening){recognitionRef.current?.stop();return;}const speechWindow=window as typeof window&{SpeechRecognition?:SpeechRecognitionConstructor;webkitSpeechRecognition?:SpeechRecognitionConstructor};const Recognition=speechWindow.SpeechRecognition??speechWindow.webkitSpeechRecognition;if(!Recognition){setVoiceMessage('Voice commands are not supported in this browser.');return;}const recognition=new Recognition();recognition.lang='en-IN';recognition.interimResults=false;recognition.continuous=false;recognition.onresult=event=>{const transcript=event.results[0]?.[0]?.transcript??'';const command=parseVoiceCashCommand(transcript);if(!command){setVoiceMessage(`Could not understand: “${transcript}”`);return;}setCounts(current=>({...current,[command.note]:command.quantity}));const amount=command.note*command.quantity;setVoiceMessage(`₹${command.note} × ${command.quantity} added`);announce(amount);};recognition.onerror=event=>setVoiceMessage(event.error==='not-allowed'?'Microphone permission was denied.':'Voice command could not be captured.');recognition.onend=()=>setListening(false);recognitionRef.current=recognition;setListening(true);setVoiceMessage('Listening…');recognition.start();};
      ...
      <div className="relative mx-auto max-w-7xl px-3 py-5 sm:px-6 sm:py-8 lg:px-8 lg:py-12">
        ...
        <div className="mt-6 grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px]">
          <div className="rounded-[28px] border border-white bg-white/80 p-3 shadow-xl shadow-slate-200/60 backdrop-blur dark:border-white/10 dark:bg-slate-900/70 dark:shadow-none sm:p-5"><div className="flex items-center justify-between gap-3 px-1 pb-4"><div><p className="text-xs font-black uppercase tracking-[0.16em] text-brand-600">Denominations</p><h2 className="mt-1 text-xl font-black text-slate-900 dark:text-white">Tap, type & count</h2></div><button disabled={totalNotes===0&&!name&&!expected} onClick={reset} className="inline-flex h-11 shrink-0 items-center gap-2 rounded-xl bg-red-600 px-4 text-xs font-black text-white shadow-lg shadow-red-600/25 transition hover:bg-red-700 active:scale-95 disabled:opacity-40"><RotateCcw className="h-4 w-4"/>Reset</button></div>
            <div className="mb-4 flex flex-col gap-3 rounded-2xl border border-brand-200 bg-gradient-to-r from-brand-50 to-cyan-50 p-3 dark:border-brand-500/20 dark:from-brand-500/10 dark:to-cyan-500/10 sm:flex-row sm:items-center sm:justify-between"><div className="flex min-w-0 items-center gap-3"><span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${listening?'animate-pulse bg-red-500 text-white':'bg-brand-600 text-white'}`}>{listening?<Volume2 className="h-5 w-5"/>:<Mic className="h-5 w-5"/>}</span><div className="min-w-0"><p className="text-sm font-black text-slate-800 dark:text-white">Voice-assisted entry</p><p className="truncate text-xs text-slate-500 dark:text-slate-400">{voiceMessage}</p></div></div><button onClick={toggleVoice} className={`inline-flex min-h-10 items-center justify-center gap-2 rounded-xl px-4 text-xs font-black transition ${listening?'bg-red-600 text-white hover:bg-red-700':'bg-white text-brand-700 shadow-sm hover:bg-brand-100 dark:bg-white/10 dark:text-brand-300'}`}>{listening?<><MicOff className="h-4 w-4"/>Stop listening</>:<><Mic className="h-4 w-4"/>Start voice</>}</button></div>
            <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3">{NOTES.map((note,index)=>{const active=counts[note]>0;return <article key={note} className={`relative overflow-hidden rounded-2xl border p-3 transition-all ${active?'border-brand-500 bg-brand-50 shadow-lg shadow-brand-500/10 dark:bg-brand-500/10':'border-slate-200 bg-white hover:border-brand-300 dark:border-white/10 dark:bg-white/[0.03]'}`}><div className="flex items-start justify-between gap-2"><div><p className={`text-xl font-black tracking-tight ${active?'text-brand-700 dark:text-brand-300':'text-slate-800 dark:text-slate-100'}`}>₹{note}</p><p className="mt-0.5 text-[10px] font-bold uppercase tracking-wider text-slate-400">Indian note</p></div><p className="max-w-[80px] truncate text-right text-xs font-extrabold tabular-nums text-slate-500 dark:text-slate-300">{money(note*counts[note])}</p></div><div className="mt-3 flex items-center rounded-xl bg-slate-100 p-1 dark:bg-black/25"><button aria-label={`Decrease ₹${note}`} onClick={()=>adjust(note,-1)} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-slate-500 transition hover:bg-white hover:text-brand-600 hover:shadow-sm dark:hover:bg-white/10"><Minus className="h-4 w-4"/></button><input data-note-index={index} aria-label={`₹${note} note quantity`} type="number" inputMode="numeric" min={0} value={counts[note]||''} placeholder="0" onChange={e=>setCount(note,e.target.value)} onKeyDown={e=>focusNext(e,index,note)} className="h-9 min-w-0 flex-1 bg-transparent px-1 text-center text-lg font-black tabular-nums outline-none"/><button aria-label={`Increase ₹${note}`} onClick={()=>adjust(note,1)} className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-brand-600 text-xl font-medium text-white shadow-md shadow-brand-600/25 transition hover:bg-brand-700 active:scale-90">+</button></div></article>})}</div>
          </div>
