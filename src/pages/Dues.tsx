...
import {
  ...
  Plus, Edit3, Trash2, ShieldAlert, Tag, Filter, X
} from 'lucide-react';
...
export default function Dues() {
  ...
  // Outstanding Employee Summary Grouping
  const employeeOutstandingSummary = useMemo<EmployeeOutstandingGroup[]>(() => {
    ...
    for (const due of dues) {
      ...
      const rem = safeAmount(due.remaining_amount);
      if (rem <= 0) continue;
      ...
      const orig = safeAmount(due.original_amount);
      const rec = safeAmount(due.recovered_amount);
    ...
    result.sort((a, b) => b.totalRemaining - a.totalRemaining || a.collectorName.localeCompare(b.collectorName));
    return result;
  }, [dues]);
  ...
  return (
    <div className="space-y-6 max-w-full overflow-x-hidden">
      ...
      {loading ? (
        ...
        <Card>
          <EmptyState icon={<AlertCircle className="h-8 w-8" />} title="No dues found" message="No matching due records found." />
        </Card>
      ) : (
      ...
      >
        <div className="space-y-4">
          ...
          <div className="space-y-2 max-h-96 overflow-y-auto">
            {filteredEmployeeSummary.map((group) => (
              <Card key={group.collectorId} className="p-3 flex justify-between items-center">
                <div>
                  <p className="font-bold">{group.collectorName} ({group.employeeId})</p>
                  <p className="text-xs text-neutral-500">{group.entriesCount} due records</p>
                </div>
                <div className="text-right">
                  <p className="font-bold text-red-500">{formatINR(group.totalRemaining)}</p>
                </div>
              </Card>
            ))}
          </div>
