  ...
  const loadFollowUps = async () => {
    ...
    try {
      const { data } = await fetchNDRShipments({
        ...
        workflowStatus: 'Reattempt Required',
        limit: 100,
      });
      setShipments(data);
    } catch (err) {
      ...
      <div className="p-4 rounded-2xl bg-[var(--card-bg)] border border-neutral-200 dark:border-neutral-800 shadow-soft flex items-center justify-between">
        <div>
          <h2 className="text-base font-bold text-neutral-900 dark:text-neutral-100 flex items-center gap-2">
            <Clock className="h-5 w-5 text-purple-500" /> Follow-up Due Queue
          </h2>
          <p className="text-xs text-neutral-500">Shipments where customer requested future delivery date or scheduled reattempt.</p>
        </div>
      ...
      <div className="rounded-2xl bg-[var(--card-bg)] border border-neutral-200 dark:border-neutral-800 shadow-soft overflow-hidden">
        {loading ? (
          <div className="py-16 text-center text-neutral-500 flex flex-col items-center gap-2">
            ...
            <span className="text-xs">Loading followups...</span>
          </div>
        ) : shipments.length === 0 ? (
          <div className="py-16 text-center text-neutral-500 text-sm">No follow-ups currently due.</div>
        ) : (
