...
import { buildRTOPreAlertMail, parseRTOPreAlertFile, parseRTOTripHtml, RTOTripDetails, RTOPreAlertResult } from '@/lib/rtoPreAlert';
...
export default function RTOPreAlertMail() {
  ...
  const mail = useMemo(() => result ? buildRTOPreAlertMail({ result, trip, footageLinks, hubName: selectedHub?.name ?? 'Hub', dispatchDate, recipientName, remarks }) : null, [result, trip, footageLinks, selectedHub?.name, dispatchDate, recipientName, remarks]);
  ...
  async function uploadTrip(file?: File) {
    if (!file) return;
    setBusy(true); setError('');
    try { setTrip(await parseRTOTripHtml(file)); toast.success('Trip sheet details extracted'); }
    catch (cause) { setTrip(null); setError(cause instanceof Error ? cause.message : 'Unable to read trip HTML'); }
