...
function TripDetailsEditor({ trip, onChange }: { trip: RTOTripDetails; onChange: (trip: RTOTripDetails) => void }) {
  ...
  return <div className="rounded-2xl border border-violet-200 bg-violet-50/50 p-4 dark:border-violet-800 dark:bg-violet-950/20"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="font-black">Trip sheet scanned</p><p className="text-xs text-neutral-500">OCR details verify karke zarurat ho to correct karein.</p></div>{missing > 0 ? <span className="rounded-full bg-warning-100 px-3 py-1 text-xs font-bold text-warning-700">{missing} required field{missing > 1 ? 's' : ''} fill karein</span> : <span className="rounded-full bg-success-100 px-3 py-1 text-xs font-bold text-success-700">Required details ready</span>}</div><div className="mt-4 grid gap-3 sm:grid-cols-3">{field('tripId', 'Trip ID', 'TR-10095812-6196')}{field('originHubCode', 'Origin hub code', 'E1/BGUS/8/JDG')}{field('destinationHubCode', 'Destination hub code', 'BGUS')}</div><div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{field('connectionId', 'Connection ID', 'Connection ID')}{field('transporterName', 'Transporter', 'Transporter name')}{field('driverName', 'Driver', 'Driver name')}{field('vehicleNumber', 'Vehicle', 'Vehicle number')}</div></div>;
}
...
export default function RTOPreAlertMail() {
  ...
  async function uploadTrip(file?: File) {
    if (!file) return;
    setBusy(true); setError('');
    try { const parsed = await parseRTOTripDocument(file); setTrip(parsed); toast.success(parsed.tripId && parsed.originHubCode && parsed.destinationHubCode ? 'Trip sheet details extracted' : 'Image read ho gayi—missing details verify karke fill karein'); }
