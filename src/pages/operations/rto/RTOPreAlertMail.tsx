...
function TripDetailsEditor({ trip, onChange }: { trip: RTOTripDetails; onChange: (trip: RTOTripDetails) => void }) {
  const field = (name: keyof RTOTripDetails, label: string, placeholder: string) => <label className="block"><span className="text-[10px] font-black uppercase tracking-wider text-neutral-500">{label}</span><input value={trip[name]} onChange={(event) => onChange({ ...trip, [name]: event.target.value })} placeholder={placeholder} className="mt-1 w-full rounded-xl border border-neutral-200 bg-white px-3 py-2.5 text-sm font-bold outline-none focus:border-brand-500 dark:border-neutral-700 dark:bg-neutral-950" /></label>;
  const missing = [trip.tripId, trip.originHubCode, trip.destinationHubCode].filter((value) => !value).length;
  return <div className="rounded-2xl border border-violet-200 bg-violet-50/50 p-4 dark:border-violet-800 dark:bg-violet-950/20"><div className="flex flex-wrap items-center justify-between gap-2"><div><p className="font-black">Trip sheet scanned</p><p className="text-xs text-neutral-500">Verify the extracted details and correct them if needed.</p></div>{missing > 0 ? <span className="rounded-full bg-warning-100 px-3 py-1 text-xs font-bold text-warning-700">Complete {missing} required field{missing > 1 ? 's' : ''}</span> : <span className="rounded-full bg-success-100 px-3 py-1 text-xs font-bold text-success-700">Required details ready</span>}</div><div className="mt-4 grid gap-3 sm:grid-cols-3">{field('tripId', 'Trip ID', 'TR-10095812-6196')}{field('originHubCode', 'Origin hub code', 'E1/BGUS/8/JDG')}{field('destinationHubCode', 'Destination hub code', 'BGUS')}</div><div className="mt-3 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{field('connectionId', 'Connection ID', 'Connection ID')}{field('transporterName', 'Transporter', 'Transporter name')}{field('driverName', 'Driver', 'Driver name')}{field('vehicleNumber', 'Vehicle', 'Vehicle number')}</div></div>;
}
...
export default function RTOPreAlertMail() {
  ...
  async function uploadTrip(file?: File) {
    if (!file) return;
    setBusy(true); setError('');
    try { const parsed = await parseRTOTripDocument(file); setTrip(parsed); toast.success(parsed.tripId && parsed.originHubCode && parsed.destinationHubCode ? 'Trip sheet details extracted' : 'Image scanned. Review and complete the missing details'); }
