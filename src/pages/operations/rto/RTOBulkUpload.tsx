    ...
    <div className="space-y-5">
      ...
      <section className="grid gap-5 xl:grid-cols-[1.25fr_.75fr]">
        <div className="rounded-3xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
          <div className="mb-3 flex items-center justify-between"><div><h3 className="font-black text-neutral-900 dark:text-white">Paste AWB Numbers</h3><p className="text-[11px] text-neutral-500">Newline, comma, space, tab, semicolon and pipe supported.</p></div><ClipboardPaste className="h-5 w-5 text-brand-600" /></div>
          <textarea value={rawInput} onChange={(event) => setRawInput(event.target.value)} placeholder={'VM1234567890\nVM1234567891, VM1234567892'} className="min-h-52 w-full resize-y rounded-2xl border border-neutral-200 bg-neutral-50 p-4 font-mono text-sm outline-none transition focus:border-brand-500 focus:ring-4 focus:ring-brand-500/10 dark:border-neutral-800 dark:bg-neutral-950" />
          <div className="mt-3 flex flex-wrap items-center justify-between gap-3"><span className="text-xs font-semibold text-neutral-500"><strong className="text-neutral-900 dark:text-white">{parsedCount}</strong> unique AWBs detected</span><div className="flex gap-2"><button onClick={clearWorkspace} className="inline-flex items-center gap-2 rounded-xl border border-neutral-200 px-4 py-2.5 text-xs font-bold dark:border-neutral-700"><RotateCcw className="h-4 w-4" /> Clear</button><button onClick={processAwbs} className="inline-flex items-center gap-2 rounded-xl bg-brand-600 px-4 py-2.5 text-xs font-black text-white shadow-lg shadow-brand-500/20"><CheckCircle2 className="h-4 w-4" /> Process & Preview</button></div></div>
        </div>

        <div className="rounded-3xl border border-neutral-200 bg-white p-5 shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
          <h3 className="font-black text-neutral-900 dark:text-white">Bulk Settings</h3><p className="mb-5 text-[11px] text-neutral-500">Apply one decision to every processed AWB.</p>
          <label className="mb-1.5 block text-xs font-bold">Default Status</label><select value={status} onChange={(event) => setStatus(event.target.value)} className="input-base mb-4">{STATUSES.map((item) => <option key={item}>{item}</option>)}</select>
          <label className="mb-1.5 block text-xs font-bold">Default Reason</label><select value={reason} onChange={(event) => setReason(event.target.value)} className="input-base mb-5">{REASONS.map((item) => <option key={item}>{item}</option>)}</select>
      ...
      <section className="overflow-hidden rounded-3xl border border-neutral-200 bg-white shadow-sm dark:border-neutral-800 dark:bg-neutral-900">
        <div className="flex flex-col gap-3 border-b border-neutral-200 p-4 sm:flex-row sm:items-center sm:justify-between dark:border-neutral-800">
          ...
          <div className="flex gap-2"><button onClick={copyAwbs} className="inline-flex items-center gap-2 rounded-xl border border-neutral-200 px-3 py-2.5 text-xs font-bold dark:border-neutral-700"><Copy className="h-4 w-4" /> Copy AWBs</button><button onClick={downloadExcel} className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2.5 text-xs font-black text-white"><Download className="h-4 w-4" /> Download Excel</button></div>
        </div>
