...
export function printDailyClosingsPdf(rows: DailyClosing[], title: string, finalization?: DailyClosingFinalization | null) {
  ...
  const estimatedHeight = 500 + (rows.length * 27) + (metrics.shortages.length * 22);
  const printScale = Math.max(0.58, Math.min(1, 970 / estimatedHeight));
  const body = rows.map((r, index) => {
    ...
    const totalVariance = cashVariance + onlineVariance;
    return `<tr><td>${index + 1}</td><td><strong>${escapeHtml(r.collector?.name ?? 'Employee')}</strong><small>${escapeHtml(r.collector?.employee_id ?? '')}</small></td><td>${formatINR(r.expected_cash)}</td><td>${formatINR(r.actual_cash)}</td><td class="${cashVariance < 0 ? 'negative' : cashVariance > 0 ? 'positive' : 'matched'}">${formatINR(cashVariance)}</td><td>${formatINR(r.expected_online_amount)}</td><td>${formatINR(r.online_amount)}</td><td class="${onlineVariance < 0 ? 'negative' : onlineVariance > 0 ? 'positive' : 'matched'}">${formatINR(onlineVariance)}</td><td class="${totalVariance < 0 ? 'negative' : totalVariance > 0 ? 'positive' : 'matched'}">${formatINR(totalVariance)}</td><td><span class="status ${escapeHtml(r.status)}">${escapeHtml(r.status.toUpperCase())}</span></td></tr>`;
  }).join('');
  ...
  *{box-sizing:border-box}html,body{margin:0;background:#eef2f7;color:#172033;font-family:Inter,Arial,sans-serif}.print-fit{transform:scale(${printScale});transform-origin:top left;width:${100 / printScale}%}.page{max-width:794px;margin:auto;background:#fff;padding:18px}.hero{display:flex;justify-content:space-between;align-items:flex-start;border-radius:14px;padding:16px 18px;color:#fff;background:linear-gradient(135deg,#111827,#312e81 60%,#6d28d9)}.eyebrow,.section-title span{font-size:9px;font-weight:800;letter-spacing:.18em}.hero h1{margin:5px 0 3px;font-size:20px}.hero p{margin:0;color:#ddd6fe;font-size:12px}.hub{text-align:right}.hub strong{display:block;font-size:17px}.hub span{font-size:10px;color:#c4b5fd}.kpis{display:grid;grid-template-columns:repeat(5,1fr);gap:6px;margin:10px 0}.kpi{padding:9px;border:1px solid #e2e8f0;border-radius:14px;background:#f8fafc}.kpi span{display:block;font-size:9px;font-weight:800;color:#64748b;text-transform:uppercase}.kpi b{display:block;margin-top:4px;font-size:13px}.kpi.cash b{color:#047857}.kpi.online b{color:#2563eb}.kpi.short b{color:#dc2626}.verification,.draft{display:flex;gap:12px;align-items:center;margin:13px 0;padding:11px 14px;border-radius:11px}.verification{border:1px solid #86efac;background:#f0fdf4}.verification span{font-size:9px;font-weight:900;color:#15803d}.verification strong{font-size:12px}.verification small{margin-left:auto;color:#64748b}.draft{border:1px solid #fbbf24;background:#fffbeb;color:#92400e;font-weight:bold;font-size:11px}.shortage{margin:10px 0;padding:10px;border:1px solid #fecaca;border-radius:16px;background:#fff7f7}.section-title{display:flex;justify-content:space-between;align-items:end}.section-title span{color:#dc2626}.section-title h2{margin:4px 0 0;font-size:16px}.section-title>strong{font-size:20px;color:#dc2626}.shortage-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:5px;margin-top:7px}.shortage article{display:flex;align-items:center;gap:6px;padding:6px;border-radius:11px;background:#fff;border:1px solid #fee2e2}.rank{display:flex;width:25px;height:25px;align-items:center;justify-content:center;border-radius:8px;background:#fee2e2;color:#dc2626;font-weight:900}.shortage h3{margin:0;font-size:11px}.shortage p{margin:3px 0 0;font-size:9px;color:#64748b}.shortage article>b{margin-left:auto;color:#dc2626;font-size:12px}.clear{display:flex;gap:12px;margin:18px 0;padding:14px;border-radius:13px;background:#ecfdf5;color:#047857}.clear span{font-size:11px;color:#64748b}table{width:100%;border-collapse:separate;border-spacing:0;margin-top:9px;font-size:8px;border:1px solid #e2e8f0;border-radius:13px;overflow:hidden}th{padding:6px;background:#111827;color:#fff;text-align:left;font-size:8px;text-transform:uppercase;letter-spacing:.08em}td{padding:5px;border-top:1px solid #e2e8f0}td small{display:block;margin-top:2px;color:#94a3b8}.negative{color:#dc2626;font-weight:800}.positive{color:#d97706;font-weight:800}.matched{color:#059669;font-weight:800}.status{padding:4px 7px;border-radius:999px;background:#e2e8f0;font-size:8px;font-weight:900}.status.approved{background:#dcfce7;color:#15803d}.status.rejected{background:#fee2e2;color:#dc2626}.footer{display:flex;justify-content:space-between;margin-top:15px;padding-top:10px;border-top:1px solid #e2e8f0;font-size:9px;color:#94a3b8}@page{size:A4 portrait;margin:6mm}@media print{html,body{width:210mm;height:297mm;overflow:hidden;background:#fff}.page{padding:0;break-inside:avoid;page-break-inside:avoid}.hero{-webkit-print-color-adjust:exact;print-color-adjust:exact}.kpi,.shortage,.clear,th,.status{-webkit-print-color-adjust:exact;print-color-adjust:exact}}
  </style></head><body><div class="print-fit"><main class="page"><header class="hero"><div><span class="eyebrow">HUB OPERATIONS · FINANCIAL CONTROL</span><h1>${escapeHtml(title)}</h1><p>Collector-wise cash and online reconciliation report</p></div><div class="hub"><span>HUB</span><strong>${escapeHtml(hubName)}</strong><span>${escapeHtml(rows[0]?.closing_date ?? '')}</span></div></header><section class="kpis"><div class="kpi"><span>Total Expected</span><b>${formatINR(metrics.totalExpected)}</b></div><div class="kpi cash"><span>Total Cash</span><b>${formatINR(metrics.totalCash)}</b></div><div class="kpi online"><span>Total Online</span><b>${formatINR(metrics.totalOnline)}</b></div><div class="kpi"><span>Total Collected</span><b>${formatINR(metrics.totalCollected)}</b></div><div class="kpi short"><span>Total Shortage</span><b>${formatINR(metrics.totalShortage)}</b></div></section>${verification}${shortage}<table><thead><tr><th>#</th><th>Employee</th><th>Cash Due</th><th>Cash Received</th><th>Cash Var.</th><th>Online Due</th><th>Online Received</th><th>Online Var.</th><th>Total Var.</th><th>Status</th></tr></thead><tbody>${body}</tbody></table><footer class="footer"><span>${rows.length} employee closing records</span><span>Generated ${new Date().toLocaleString('en-IN')}</span></footer></main></div><script>window.onload=()=>window.print()</script></body></html>`);
  win.document.close();
}
